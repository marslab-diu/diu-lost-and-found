...
const ReportLost = () => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            console.log('Report Lost Data:', submitData);

            const response = await axiosSecure.post('/lost-reports', submitData);
            if (response.data.success) {
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div>
                        <textarea
                            ...
                            placeholder="Describe your lost item in detail (brand, model, distinguishing features, etc.)"
                            rows={4}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label htmlFor="lost_date" className="block text-sm font-medium text-gray-700 mb-1">
                                Lost Date
                            </label>
                            <input
                                type="date"
                                ...
                                {...register('lost_date', { 
                                    ...
                                    validate: (value) => {
                                        ...
                                        return selectedDate <= today || 'Lost date cannot be in the future';
                                    }
                                })}
                                max={new Date().toISOString().split('T')[0]}
                                className={`input focus:outline-none focus-within:outline-none input-bordered w-full ${
                                    errors.lost_date ? 'input-error' : ''
                                }`}
                            />
                            {errors.lost_date && (
                                <p className="text-red-500 text-sm mt-1">{errors.lost_date.message}</p>
                            )}
                        </div>

                        <div>
                            <label htmlFor="lost_time" className="block text-sm font-medium text-gray-700 mb-1">
                                Lost Time (Approximate)
                            </label>
                            <input
                                type="time"
                                id="lost_time"
                    ...
                    <div>
                        <input
                            ...
                            {...register('lost_location', { 
                                required: 'Lost location is required',
                                minLength: {
                                    value: 3,
                            ...
                            className={`input focus:outline-none input-bordered w-full ${
                                errors.lost_location ? 'input-error' : ''
                            }`}
                        />
                        {errors.lost_location && (
                            <p className="text-red-500 text-sm mt-1">{errors.lost_location.message}</p>
                        )}
                    </div>
